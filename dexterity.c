...
//                                   1 = TRUE       0 = FALSE                                       //
//                                                                                                  //
  static XDATA const char transmitter_id[] = "6K6ST";                                               //
//                                                                                                  //
...
//    U1CSR &= ~0x40; // Recevier disable
    U0UCR &= ~0x40; //CTS/RTS Off PFM
    U0CSR &= ~0x40; // Recevier disable PFM
}
...
void print_packet(Dexcom_packet* pPkt) {
    ...
    uartEnable();
///    setDigitalOutput(12,HIGH); //PM uartrx
    delayMs(40); //PM uartrx
///    setDigitalOutput(12,LOW); //PM uartrx
//    printf("6D09B %lu %lu %hhu %d %hhu %d\r\n", 
...
//    	adcConvertToMillivolts(adcRead(0)));
    printf("6K6ST %06lu %06lu %03hhu %03d %02hhu\r\n", 
    	dex_num_decoder(pPkt->raw), 
    	2*dex_num_decoder(pPkt->filtered), 
    	pPkt->battery, 
    	pPkt->RSSI, 
    ...
    printf("6K6ST %06lu %06lu %03hhu %03d %02hhu\r\n", 
    	...
    	pPkt->RSSI, 
    	txid); //PM uartrx
//    printf("%06lu%06lu%03hhu%03d%02hhu\r\n", 
//    	dex_num_decoder(pPkt->raw), 
//    	2*dex_num_decoder(pPkt->filtered), 
//    	pPkt->battery, 
//    	pPkt->RSSI, 
...
//    printf("AT+NAMEHANKJr\r\n");
///    setDigitalOutput(12,HIGH); //PM uartrx
    delayMs(4); //PM uartrx
///    setDigitalOutput(12,LOW); //PM uartrx
    printf("20208018444821608712\r\n"); //PM uartrx
    printf("20208018444821608712\r\n"); //PM uartrx
//    printf("20208018444821608712\r\n"); //PM uartrx
//    printf("20208018444821608712\r\n"); //PM uartrx
    uartDisable();
...
//    configBt();
    ...
    while(1) {
        ...
        // delay to give time to send packet via UART
        delayMs(30000);
